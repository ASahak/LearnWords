...
const Register: React.FC<{ navigation: any }> = ({ navigation }) => {
  ...
  const { handleSubmit, watch, control, formState, getValues, reset } =
  ...
  const onSubmitRegister = async (data: RegisterInputs) => {
    try {
      setIsLoading(true);
      const { msg, type } = await Firebase.registerUser(
        data.email,
        data.password,
        data.username,
      );
    ...
    <View style={styles.mainBody}>
      <ScrollView>
        <View style={{ ...styles.createTitleView, marginBottom: 70 }}>
          ...
          <Text style={styles.createToContinue}>
            Create to make your life easy
          </Text>
        </View>
